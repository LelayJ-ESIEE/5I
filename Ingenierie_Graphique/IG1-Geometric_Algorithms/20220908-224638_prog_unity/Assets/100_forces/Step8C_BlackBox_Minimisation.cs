...
//
public class Step8C_BlackBox_Minimisation : MonoBehaviour
{
	/// notre graph
	...
	public void doOneStep()
	{
		...
		// on bouge selon x et on avance de la quantite qui minimise le mieux
		for (int p = 0; p < nbPoints; ++p)
		{
			...
			float test_quantity = 0.01f;
			float newValue = fonctionAMinimiser(P + Vector3.right * test_quantity);
			float evolutionValue = newValue - currentValue;

			// resolution en utilisant une droite : x * a + b = y. on connait 2 points de la droite :
			//   avec x : P.x           et        (P.x + test_quantity)
			//   avec y : currentValue  et        newValue
			float coeff_directeur_A = evolutionValue / test_quantity; // (y2-y1) / (x2-x1)

			if (Mathf.Abs(coeff_directeur_A) < 0.01f)
			{
				// on n'a pas suffisamment evolue, droite parallele donc on ne va pas tenter
				continue;
			}

			float ordonnee_origine_B = currentValue - (P.x * coeff_directeur_A);
			// maintenant ou est-ce que Y vaut 0 ?
			//  on a : x_inconnu * a + b = 0
			//  donc : x_inconnu * a = -b 
			//  donc : x_inconnu = -b/a
			float perfectX = -ordonnee_origine_B / coeff_directeur_A;

			// soit on assigne le mouvement parfait
			P.x = perfectX;

			// soit on fait juste un bout du mouvement car on a potentiellement aussi des mouvements a faire sur y!
			//P.x = P.x *0.5f + perfectX * 0.5f;
		}

		// on bouge selon y et on avance de la quantite qui minimise le mieux
		for (int p = 0; p < nbPoints; ++p)
		{
			GVector3 P = mGraph.mPoints[p];
			float currentValue = fonctionAMinimiser(P);
			...
			float test_quantity = 0.01f;
			float newValue = fonctionAMinimiser(P + Vector3.up * test_quantity);
			float evolutionValue = newValue - currentValue;

			// resolution en utilisant une droite : x * a + b = y. on connait 2 points de la droite :
			//   avec x : P.y           et  (P.y + test_quantity)
			//   avec y : currentValue  et      newValue
			float coeff_directeur_A = evolutionValue / test_quantity; // (y2-y1) / (x2-x1)
			...
			float ordonnee_origine_B = currentValue - (P.y * coeff_directeur_A);
			// maintenant ou est-ce que Y vaut 0 ?
			//  on a : x_inconnu * a + b = 0
			//  donc : x_inconnu * a = -b 
			//  donc : x_inconnu = -b/a
			...
			// soit on fait juste un bout du mouvement car on a aussi des mouvements a faire sur x!
			//P.y = P.y * 0.5f + perfectX * 0.5f;
		}
