...
public class Triangle3D
{
	...
	/// les coordonnees minimum (aabb) de ce triangle
	public Vector3 getMinimum()
	{
		float x = Mathf.Min(A.x, B.x, C.x);
		float y = Mathf.Min(A.y, B.y, C.y);
		float z = Mathf.Min(A.z, B.z, C.z);
		return new Vector3(x,y,z);
	}

	/// les coordonnees maximum (aabb) de ce triangle
	public Vector3 getMaximum()
	{
		float x = Mathf.Max(A.x, B.x, C.x);
		float y = Mathf.Max(A.y, B.y, C.y);
		float z = Mathf.Max(A.z, B.z, C.z);
		return new Vector3(x,y,z);
	}
	...
	/// ex: 1 : la taille reste la meme
	/// ex: 0.5 : la taille est divisee par 2
	public Triangle3D getMultiplied(float f01x, float f01y, float f01z)
	{
		var lNewTri = new Triangle3D(this);
		lNewTri.A = new Vector3(A.x * f01x, A.y * f01y, A.z * f01z); 
		lNewTri.B = new Vector3(B.x * f01x, B.y * f01y, B.z * f01z); 
		lNewTri.C = new Vector3(C.x * f01x, C.y * f01y, C.z * f01z); 
		return lNewTri;
	}
	...
	/// cree une copie qui a ete decalee par translation
	public Triangle3D getTranslated(float pX, float pY, float pZ)
	{
		Vector3 lNewA = new Vector3( pX + A.x, pY + A.y, pZ + A.z);
		Vector3 lNewB = new Vector3( pX + B.x, pY + B.y, pZ + B.z);
		Vector3 lNewC = new Vector3( pX + C.x, pY + C.y, pZ + C.z);
		return new Triangle3D( lNewA, lNewB, lNewC);
	}
