...
public class Mesh3D
{
	...
	{
		var lMatrix = Matrix4x4.TRS( new Vector3(pX,pY,pZ), Quaternion.identity, Vector3.one);
		for(int i = 0; i < mPoints.Count; ++i)
		{
			mPoints[i] = lMatrix.MultiplyPoint( mPoints[i] );
		}
	...
	/// conversion de mFaces.
	/// cree un tableau avec la liste des triangles en tant que int
	public int[] getFacesAsInts()
	{
		int[] result = new int[mFaces.Count * 3];
		for(int f = 0; f < mFaces.Count; f++)
		{
			var lFace = mFaces[f];
			int f3 = 3*f;
	...
	/// renvoie 1 tableau avec 2 Vector.
	///  [0] : les valeurs minimum
	///  [1] : les valeurs maximum
	/// les points qui n'ont pas de triangles ne sont pas comptabilises
	public Vector3[] getMinMaxOfTriangles()
	{
		...
		foreach( var lFace in mFaces)
		{
			var A = mPoints[lFace.a];
			var B = mPoints[lFace.b];
			var C = mPoints[lFace.c];

			minimum.x = Mathf.Min(A.x, B.x, C.x, minimum.x); 
			minimum.y = Mathf.Min(A.y, B.y, C.y, minimum.y); 
			minimum.z = Mathf.Min(A.z, B.z, C.z, minimum.z); 

			maximum.x = Mathf.Max(A.x, B.x, C.x, maximum.x); 
			maximum.y = Mathf.Max(A.y, B.y, C.y, maximum.y); 
			maximum.z = Mathf.Max(A.z, B.z, C.z, maximum.z); 
		}
