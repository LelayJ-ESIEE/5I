...
public class GVector3
{
	...
	/// obtenir les valeurs minimums pour cette liste de points
	public static Vector3 sGetMinimumOf( List<Vector3> pPoints )
	{
		Vector3 result = new Vector3( float.MaxValue, float.MaxValue, float.MaxValue );
		foreach( var p in pPoints)
		{
			result.x = Mathf.Min( result.x, p.x);
			result.y = Mathf.Min( result.y, p.y);
			result.z = Mathf.Min( result.z, p.z);
		}
		return result;
	}

	/// obtenir les valeurs maximum pour cette liste de points
	public static Vector3 sGetMaximumOf( List<Vector3> pPoints )
	{
		Vector3 result = new Vector3( float.MinValue, float.MinValue, float.MinValue );
		foreach( var p in pPoints)
		{
			result.x = Mathf.Max( result.x, p.x);
			result.y = Mathf.Max( result.y, p.y);
			result.z = Mathf.Max( result.z, p.z);
		}
		return result;
	}
