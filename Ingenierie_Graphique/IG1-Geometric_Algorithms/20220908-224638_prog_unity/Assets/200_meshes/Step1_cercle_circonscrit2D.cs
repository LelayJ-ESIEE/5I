...
public class Step1_cercle_circonscrit2D : MonoBehaviour 
{
	/// notre graph
	...
	public static Vector3 calcCircumcircle_center_v2( Vector3 A, Vector3 B, Vector3 C)
	{
		...
		Vector3 center = Vector3.Cross((a.sqrMagnitude * b - b.sqrMagnitude*a), a_cross_b) / (2* a_cross_b.sqrMagnitude)  + C;
		return center;
		//TOREMOVEFOREXERCISES_END return Vector3.zero;
	}
	...
	public static bool isInCircumcircle_XY(Vector3 A, Vector3 B, Vector3 C, Vector3 P)
	{
		...
		var lB = B;
		var lC = C;
		if( !isTriangleTrigo_XY(A,B,C) )
		{
			// on echange le sens pour qu'il soit trigo
			lB = C;
			lC = B;
		}
		...
		var matrice_rowMajor = new float[9]{ 
			PA.x, PA.y, PA.x * PA.x + PA.y * PA.y,  // 0 1 2 
			PB.x, PB.y, PB.x * PB.x + PB.y * PB.y,  // 3 4 5 
			PC.x, PC.y, PC.x * PC.x + PC.y * PC.y}; // 6 7 8 

		// alias. cela sera plus simple d'ecrire la formule du determinant ensuite avec cela
		var m = matrice_rowMajor;
		...
		return determinant > 0.001f; // on met un peu marge pour eviter les erreurs sur les quadrilateres symetriques
