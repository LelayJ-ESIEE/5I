...
/// 
public class Step6_voronoi : MonoBehaviour
{
	...
	public static Graph voronoi_from_delaunay(List<Triangle3D> pDelaunay)
	{
		...
		for(int t = 0; t < pDelaunay.Count; ++t)
		{
			var triangle = pDelaunay[t];
			var sphere = Step5_sphere_circonscrit3D.getCircomcircleSphere( triangle.A, triangle.B, triangle.C);
			voronoi_result.mPoints.Add(  sphere.center );
		}

		for(int t1 = 0; t1 < pDelaunay.Count; ++t1)
		{
			var T1 = pDelaunay[t1];
			for(int t2 = t1+1; t2 < pDelaunay.Count; ++t2)
			{
				var T2 = pDelaunay[t2];
				if( T1.hasAtLeastOneEdgeInCommon_nonOriented( T2))
				{
					voronoi_result.addLinkIfNotThereBetween( t1, t2, false);
				}
			}
		}
