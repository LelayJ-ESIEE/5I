...
/// 
/// 
public class Step3_ik_descenteDeNewton : MonoBehaviour
{
	...
	public void doOneCycle()
	{
		...
		for(int i = mChain.Count -1 ; i >= 0; --i)
		{
			...
			{
				float previousValue = fonctionDeCout();

				if( Mathf.Abs(previousValue) < 0.01f )
				{
					// on est deja tres proche
					//break;
				}

				Vector3 angleVariation = angleVariations[e];
				Vector3 lCurrentAngles_deg = lBone.eulerAngles;

				lBone.Rotate( angleVariation );

				float newValue = fonctionDeCout();

				// comment cela a evolue
				float evolution_du_cout = newValue - previousValue;

				if( Mathf.Abs(evolution_du_cout) < 0.01f )
				{
					// pas de changement de toute facon
					continue;
				}

				lBone.eulerAngles = lCurrentAngles_deg;

				//
				// "regle de trois" :
				//
				//  angleVariation ===> variation_couts
				//      ???	  		===> -cout_depart
				//
				// Donc
				//      ??? = angleVariation * (-cout_depart) / variation_couts
				//

				Vector3 angleToApply = (angleVariation * (-previousValue) / evolution_du_cout);


				// evidemment cela va partir en vrille si on ne met pas des limites, surtout avec des angles!
				// sinon on aura tres facilement des angles de plus de 360 degres!
				float limitingAngle = 5.0f;
				angleToApply.x = Mathf.Clamp( angleToApply.x, -limitingAngle, limitingAngle);
				angleToApply.y = Mathf.Clamp( angleToApply.y, -limitingAngle, limitingAngle);
				angleToApply.z = Mathf.Clamp( angleToApply.z, -limitingAngle, limitingAngle);

				lBone.Rotate( angleToApply ); 
			}
		}
